...
$app->post('/position_log', function() {
	...
	log_access($conn, 'log_position', $user_data['row_id'], $_POST['longitude'], $_POST['longitude']);
	echo json_encode($data);
});
...
$app->post('/upload_logs', function() {
	...
	$log_files_dir = 'uploads/'.$user_id.'/log_files';

	if (!file_exists($log_files_dir)) {
		mkdir($log_files_dir, 0777, true);
	}
