...
function get_updated_route_data($conn = "", &$data, $user_id) {
	...
	$query = "select vda.*, mr.route_code as actual_route_code, vo.operator_name, vo.operator_address1, vo.operator_address2, vo.operator_city, vo.operator_helpline, vm.vehicle_number, vm.vehicle_type, vt.vehicle_basefare  from `vehicle_device_access` vda JOIN `master_routes` mr ON mr.row_id = vda.route_id JOIN `vehicle_operators` vo ON vo.row_id = vda.operator_id JOIN `vehicle_master` vm ON vm.row_id = vda.vehicle_id JOIN `vehicle_types` vt ON vt.vehicle_type = vm.vehicle_type WHERE vda.row_id = '".$user_id."'";
		...
		if($result->num_rows > 0) {
			...
			$data['is_multiroute']	= $user_data['is_multiroute'];
			...
			//-----------------------------------------------------------------------------------------------------------------------------------------------------------
				//Get operators routes list data 
				$routes_list_query = "select * from `master_routes` WHERE operator_id = '".$user_data['operator_id']."' AND route_status = 'ACTIVE'";

				$data['routes_list_query'] = $routes_list_query;

				$routes_list_result = $conn->query($routes_list_query);

				$routes_list = array();

				if($routes_list_result->num_rows > 0) {

					while ($row = $routes_list_result->fetch_assoc()){
						$routes_list[$row['route_code']] = $row['route_code'];

						$routes_list_new[] = 
						 array('key' => $row['route_code'], 'route' => $row['route_code'] ." [".$row['route_start_stage'] ." - ".$row['route_end_stage']."]");
					}
				}

				$data['routes_list'] = $routes_list;

				$data['routes_list_new'] = $routes_list_new;
			//-----------------------------------------------------------------------------------------------------------------------------------------------------------
...
//Location log of the vehicle
$app->post('/update_route', function() {
	require_once('db.php');

	

	$master_route_query = "SELECT * FROM `master_routes` WHERE `route_code` = '".$_POST['selected_route']."'";

		
		$master_route_result = $conn->query($master_route_query);

		while ($row = $master_route_result->fetch_assoc()){
			$route_id = $row['row_id'];
		}

	$sql = "UPDATE `vehicle_device_access` SET `route_id` = '".$route_id."'"; 
	
	$sql .= " WHERE `row_id` = '".$_POST['user_id']."'";	
	...
	get_updated_route_data($conn, $data, $_POST['user_id']);

	log_access($conn, 'update_route', $_POST['user_id'], $_POST['longitude'], $_POST['longitude']);
